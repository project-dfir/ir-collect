...
| C2 | **Host reboots mid-run** | `qm reset <vmid>` during an `-Auto` run — assert files are being written first | Partial bundle parseable; `-Resume` finishes it | ⬜ **attempted 2026-07-28, HARNESS INVALID — not a collector finding.** The live-condition assertion correctly REFUSED to reset (files=0), so nothing false was recorded. Two harness defects found: (1) inline nested PowerShell over ssh mangled both the launcher and the probe — every guest action must be a script file; (2) after fixing that, a backgrounded `Start-Process` run still wrote nothing while an identical FOREGROUND run works (verified: steps 001-005 written, RAM capture started). Isolate the background-launch difference before retrying. `-Resume` is already validated via C1 |
