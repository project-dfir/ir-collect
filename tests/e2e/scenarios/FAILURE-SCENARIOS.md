...
| B2 | **Destination full** | Linux: 3 MB loop fs. Windows: 40 MB VHD filled to <96 KB free — **and assert a 512 KB write is refused before judging** | Classify structurally, honest remediation, rollup survives off-medium, ledger stays valid JSONL, verdict `destination-full` | ✅ Linux. ⚠️ **Windows: 4th attempt finally induced the condition (0 KB free, write refused) and found the fallback does NOT help** — on a destination that is full from the start the collector dies before `Invoke-Seal`, so `run_state.json` is never written and the ENOSPC rollup fallback (which lives inside seal) never fires. Operator gets 7 files and no diagnosis. Fixed by refusing up front: destination preflight requires 64 MB free, else exit 40. Healthy-destination regression verified (exit 0, seals clean); **the refusal path itself is not yet observed — verify next iteration** |
