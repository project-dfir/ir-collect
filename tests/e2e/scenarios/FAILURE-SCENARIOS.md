...
| # | Scenario | Reproduce | Correct behaviour | Status |
|---|---|---|---|---|
| C1 | **Killed mid-run** — EDR live-response harnesses commonly cap child tools at ~30 min | `Stop-Process -Force` / `kill -9` **during Stage 2** — must be a job that actually runs long enough | Always-seal wrapper produces a usable partial bundle; `-Resume` picks up the rest | ⬜ **attempted TWICE 2026-07-28, BOTH INVALID** — RapidOnly with no imager staged completes in <25s, so the kill landed after the run had already sealed. Second attempt staged an imager and waited for `memory.raw` to grow — but a 6 GB capture on WS02 finishes in under a minute, so that run also sealed before the kill (the 6144 MB image was COMPLETE, not partial). **A RapidOnly collection on these VMs is simply too fast to interrupt.** The valid test needs `-Auto` (Stage 2 artifacts/eventlogs run 13–20 min) with the kill at ~60s, which is why this has not been closed yet. NOTE: a hard `Stop-Process -Force` does not run PowerShell `finally`, so the seal wrapper is expected NOT to fire — the real question is whether the append-only ledger lets `-Resume` recover |
| C2 | **Host reboots mid-run** | `qm reset <vmid>` | Partial bundle is still parseable; resume works | ⬜ — but `-Resume` itself is now validated incidentally: on a re-run it reported ok=35 **skipped=32** verdict=COMPLETE, correctly skipping already-satisfied steps |
