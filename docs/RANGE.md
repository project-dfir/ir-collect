...
| **Read-only ISO** | attach the kit ISO; run `collectors/loader.*`; write to an `EVIDENCE`-labeled disk | — |
...
e.g. `cd kit && zip -r ../kit.zip .` (Windows guest) or `cd kit && tar czf ../kit.tgz .` (Linux guest) - build from the `collectors/` **contents** so `IR-Collect.ps1`/`ir-collect.sh` land at the archive root where the deploy scripts expect them; pass it with `-k`.
