...
5. **Credential/token stores:** **DPAPI master keys** (paths now inventoried in `00_metadata/other_encryption_keys.txt`; the blobs themselves still need copying - without them copied browser logins are undecryptable), Credential Manager/Vault, cloud CLI creds (`~/.aws`, `~/.azure`, `~/.config/gcloud`, `~/.kube/config`), Windows `~/.ssh`, PuTTY/WinSCP saved sessions, `.git-credentials`.
...
- **Encryption keys while live (now captured):** order of volatility applies to KEYS, not just data - power the box off and the master key is gone from kernel memory, leaving the image unreadable unless a custodian supplies a passphrase. Captured into `00_metadata`: Windows = BitLocker status, machine-readable recovery passwords (`bitlocker_recovery_keys.csv`), **key packages** for `repair-bde`, EFS certs, DPAPI master-key paths; Linux = the live dm-crypt **volume master keys** (`dmsetup table --showkeys`), **LUKS header backups** (without the header even a correct passphrase fails, and header destruction is a known ransomware move), crypttab/keyfile inventory, kernel keyring. Both emit `DECRYPTION-KEYS.md` with verified commands for applying them later. These outputs ARE the keys to the evidence - handle the bundle at the classification of the data it protects; `--no-keys` / `-NoKeyCapture` opts out where that is out of scope. The gate goes **AMBER** on *encrypted disk + no verified RAM*.
